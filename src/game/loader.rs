...
pub fn load_texture(path: &str) -> u32 {
    ...
    unsafe{ 
        ...
        gl::TexImage2D(gl::TEXTURE_2D, 0,gl::RGB as i32, dim.0 as i32, dim.1 as i32, 0, gl::BGRA, gl::UNSIGNED_BYTE,im.raw_pixels().as_mut_ptr() as *mut std::ffi::c_void);
...
fn fill_vtn_vectors(vertices_indexed:&Vec<Vertex>,texture_data_indexed:&Vec<TVertex>,normals_indexed:&Vec<Vertex>, indices: &Triangles)
    ...
    for triangle in indices.vertices.iter() {
        ...
        let v_i = vtn_i.0;
        let t_i = vtn_i.1.unwrap();
        let n_i = vtn_i.2.unwrap();

        vert.push(vertices_indexed[v_i].x as f32);
        vert.push(vertices_indexed[v_i].z as f32);
        vert.push(vertices_indexed[v_i].y as f32);
        tex.push(texture_data_indexed[t_i].x as f32);
        tex.push(texture_data_indexed[t_i].y as f32);
        norm.push(normals_indexed[n_i].x as f32);
        norm.push(normals_indexed[n_i].z as f32);
        norm.push(normals_indexed[n_i].y as f32);
        int.push(i);
        i+=1;
